...
- [2026-05-27 04:17:00 +03] system:architecture `client/src/services/securityBootstrap.js` و`client/src/services/apiClient.js` و`client/src/services/runtimeTelemetry.js` و`client/src/main.jsx` - إضافة Security Bootstrap Phase المعماري الصحيح: Resolve Identity → Attach Token → Initialize Correlation Context → THEN start telemetry + API clients، مع centralized auth context injection ومعالجة 401 كـ unauthenticated state وليس crash.
- [2026-05-27 01:32:00 +03] system:feature `client/src/services/correlationLayer.js` و`client/src/hooks/useRuntimeInspector.js` و`client/src/components/RuntimeInspectorPanel.jsx` - إضافة Phase 3 للـ GPS: sampling per route، حدود active/session spans، dropped span counters، وتمرير client spans إلى Runtime Inspector مع Trace Waterfall يبرز البطء والأخطاء.
- [2026-05-27 01:20:00 +03] system:feature `client/src/services/correlationLayer.js` و`client/src/main.jsx` - تقوية Correlation Layer (GPS) بإضافة traceparent headers، spans تلقائية للـ fetch، span events، critical path/summary أعمق، وربطه بنفس session/trace الخاص بالـ runtime telemetry.
- [2026-05-27 01:12:00 +03] system:fix `client/src/hooks/useAutoSave.js` - إضافة singleton registry باسم `window.__OCTOPUS_AUTOSAVE__` لمنع تكرار autosave intervals عند reload/StrictMode، مع timer واحد يستخدم آخر نسخة من callback.
- [2026-05-27 01:05:00 +03] system:fix `server/services/rateLimitService.js` و`server/index.js` و`server/services/rateLimitService.test.js` - إضافة server-side kill switch خاص بـ `/api/events` يوقف spam فوق 10 طلبات/ثانية برسالة `EVENTS_RATE_LIMIT_TRIGGERED` مع `Retry-After`، بدون إدخال HUD في rate limit الطويل.
- [2026-05-27 00:58:00 +03] system:test `server/services/rateLimitService.test.js` - إضافة regression test يثبت أن HUD event reads وtelemetry batches لا تتأثر بالـ rate limiter حتى عندما تكون حدود API منخفضة جداً.
- [2026-05-27 00:52:00 +03] system:fix `client/public/dev-hud.js` و`server/services/rateLimitService.js` - منع HUD runtime errors polling من توليد 429 عبر singleton registry في `window.__HUD_EVENTS_INSTANCES__`، تنظيف interval القديم عند reload، backoff عند 429، واستثناء قراءة `/api/events` من rate limit.
- [2026-05-27 00:45:00 +03] system:fix `server/services/rateLimitService.js` و`client/src/services/runtimeTelemetry.js` و`client/src/hooks/useExtensionsManager.js` و`client/src/components/ExtensionsPanel.jsx` - كسر حلقة 429 عبر استثناء telemetry stream/batch من rate limit، منع telemetry من تسجيل طلباته، إسقاط batches الفاشلة بدل إعادة إرسالها، caching لقائمة الإضافات، وdebounce للبحث في OpenVSX.
- [2026-05-27 00:36:00 +03] system:debug `client/src/services/runtimeTelemetry.js` و`client/src/hud/hud-ws-client.js` و`client/public/dev-hud.js` - تهيئة `window.__OCTOPUS_LAST_PATCH__` افتراضياً بحالة `skipped` ورسالة "No patch operation has run yet" حتى لا يظهر `undefined` قبل أول عملية patch.
- [2026-05-27 00:28:00 +03] system:debug `client/src/hud/hud-ws-client.js` و`client/src/auditor/useLayoutAuditor.js` و`client/public/dev-hud.js` - إضافة `window.__OCTOPUS_LAST_PATCH__` كحالة debug عالمية لآخر patch proposal/preview/file apply مع status وsource وmessage وcode/targetFile عند توفرها.
- [2026-05-27 00:20:00 +03] system:ui `client/public/dev-hud.js` و`client/public/dev-hud.css` و`client/src/hud/hud-ws-client.js` و`server/routes/hudApplyPatch.js` و`server/routes/hudAiFix.js` - تحويل HUD AI Fix إلى status-based UX بحالات صريحة `success/applied/skipped` من الباكند حتى الواجهة، مع badges ورسائل موحدة لتوليد الاقتراح، المعاينة الحية، وتطبيق الملف.
- [2026-05-27 00:10:00 +03] system:feature `client/public/dev-hud.html` و`client/public/dev-hud.js` و`client/public/dev-hud.css` و`client/src/services/runtimeTelemetry.js` - إضافة لوحة Runtime Errors داخل Engineer HUD تعرض أخطاء وتحذيرات الواجهة مع file/line/column من stack trace أو browser error payload، وتعرض آخر النقرات والطلبات قبل الخطأ كـ breadcrumbs.
- [2026-05-26 23:55:00 +03] system:feature `client/src/services/runtimeTelemetry.js` و`client/src/main.jsx` و`client/src/components/ErrorBoundary.jsx` و`server/routes/events.js` - إضافة Runtime Telemetry يلتقط نقرات المستخدم، console warnings/errors، أخطاء React، unhandled promises، وفشل الشبكة/الموارد ويرسلها كأحداث client batch إلى `/api/events/batch` مع breadcrumbs دقيقة قبل كل خطأ.
- [2026-05-26 24:45:00 +03] system:feature `client/src/hud/hud-ws-client.js` و`client/public/dev-hud.js` - إضافة `applyPatchToFile()` وزر "Apply to file" في HUD: يكتب الـ CSS مباشرة في الملف الفعلي عبر `/api/hud/apply-patch`، يعرض Saved ✅ badge + file preview بعد النجاح، مع إبقاء "Preview" (BroadcastChannel) مستقلاً.
- [2026-05-26 24:30:00 +03] system:feature `server/routes/hudApplyPatch.js` - إضافة endpoint جديد `POST /api/hud/apply-patch` يكتب patch CSS مباشرة في ملفات `client/src` مع ثلاث حالات (replaced/added/appended)، حماية path traversal، وlogs في HUD WebSocket. 12 unit test يمر.
- [2026-05-26 24:00:00 +03] system:feature `server/routes/hudAiFix.js` و`client/src/hud/hud-ws-client.js` و`client/public/dev-hud.js` و`client/src/auditor/useLayoutAuditor.js` - إضافة Groq AI Fix Engine للـ HUD: endpoint جديد مع fallback patches لـ 13 قاعدة، `requestAIFix` و`collectAffectedElements` في client، patch preview مع Analysis وConfidence badge وزرا Apply/Copy، وcss-patch-apply handler في main app عبر BroadcastChannel.
- [2026-05-26 23:58:00 +03] system:fix `server/runtime/extensionHost/vscodeShim.js` - إضافة property access للـ `workspace.getConfiguration()` ودعم `registerTextDocumentContentProvider` و`env.appRoot` حتى يتجاوز Todo Tree مرحلة activate بنجاح.
- [2026-05-26 23:06:00 +03] system:fix `server/runtime/extensionHost/vscodeShim.js` و`server/services/shimPolyfillService.js` - إضافة shims آمنة لـ `workspace.onDidCreateFiles/onDidDeleteFiles/onDidRenameFiles` لمعالجة فجوة Blackbox file events.
- [2026-05-26 23:01:00 +03] system:fix `server/services/shimPolyfillService.js` - توسيع recipe الخاص بـ `createTextEditorDecorationType` ليقبل أي إضافة تطلب API تزيين المحرر بدل إرجاع 422.
- [2026-05-26 22:58:00 +03] system:fix `server/runtime/extensionHost/vscodeShim.js` و`server/services/shimPolyfillService.js` - إضافة polyfill لـ `window.createTextEditorDecorationType` ومعالجة 422 في طلبات إصلاح Blackbox shim بدون رسائل `undefined`.
- [2026-05-26 23:52:00 +03] system:fix `server/runtime/extensionHost/vscodeShim.js` و`server/services/shimPolyfillService.js` - تصحيح Todo Tree shim ليغطي مفاتيح configuration المتداخلة مثل `highlights.customHighlight` ويدعم القراءة بالـ section أو full key.
- [2026-05-26 23:40:00 +03] system:feature `server/services/packageService.js` و`server/routes/packages.js` و`client/src/components/ExtensionsPanel.jsx` - إضافة تثبيت VSIX محلي من جهاز المستخدم عبر upload خام آمن يعيد استخدام مفكك VSIX الحالي ويسجل manifest/capabilities داخل Octopus.
- [2026-05-26 23:24:00 +03] system:feature `server/services/vscodeTestElectronService.js` و`server/runtime/vscodeTestElectron/activationTestRunner.js` و`server/services/extensionHostService.js` - إضافة `@vscode/test-electron` كـ base اختياري لتفعيل الإضافات داخل VS Code Extension Host الحقيقي عبر `OCTOPUS_EXTENSION_RUNTIME=vscode-test-electron` مع fallback shim الحالي.
- [2026-05-26 23:08:00 +03] system:feature `server/services/shimPolyfillService.js` و`server/routes/shim.js` و`server/runtime/extensionHost/vscodeShim.js` و`client/src/components/RightPanel.jsx` - إضافة pipeline آمن لتطبيق polyfill معروف لفشل Todo Tree من زر داخل رسالة الشات مع ملف generated shim polyfills واختبار regression.
- [2026-05-26 22:48:00 +03] system:fix `server/services/extensionHostService.js` و`client/src/App.jsx` و`client/src/components/EditorWorkspace.jsx` - حفظ خطأ فشل Extension Runtime بعد خروج الـ host وعرضه من logs عند الحاجة مع منع تكرار رسالة الشات وترتيب زر طلب shim.
- [2026-05-26 22:34:00 +03] system:feature `client/src/hooks/useExtensionsManager.js` و`client/src/components/EditorWorkspace.jsx` و`client/src/App.jsx` - تحويل فشل Extension Runtime إلى رسالة Octopus تفاعلية مع زر لطلب صياغة shim/polyfill من داخل بطاقة الإضافة.
- [2026-05-26 22:26:00 +03] system:debug `server/services/extensionHostService.js` و`server/runtime/extensionHost/vscodeShim.js` - طباعة stdout/stderr وactivation failures للـ extension host في PowerShell وإضافة shims أولية لـ TreeItem وworkspace APIs الشائعة.
- [2026-05-26 22:18:00 +03] system:fix `client/src/components/ExtensionsPanel.jsx` - جعل مفاتيح نتائج OpenVSX مركبة وفريدة لمنع تحذير React عند تكرار أسماء إضافات مثل `todolist`.
- [2026-05-26 22:12:00 +03] system:fix `server/projectMapEngine.js` و`server/routes/workspace.js` و`server/index.js` - منع أخطاء `EPERM watch` على Windows من إسقاط السيرفر عبر تجاهل permission watcher errors وتسجيلها كتحذير.
- [2026-05-26 22:05:00 +03] system:debug `server/runtime/extensionHost/extensionHost.js` و`server/runtime/extensionHost/vscodeShim.js` - إضافة diagnostics واضحة لتحميل extension entrypoint مع fallback لـ ESM وإضافة Tree View shim APIs الأولية.
- [2026-05-26 21:52:00 +03] system:feature `server/services/extensionHostService.js` و`server/runtime/extensionHost/*` و`client/src/components/EditorWorkspace.jsx` - إضافة Extension Host تجريبي يشغل `activate()` داخل child process مع `vscode` shim وتسجيل commands وعرض حالة runtime من واجهة الإضافات.
- [2026-05-26 21:36:00 +03] system:fix `client/src/hooks/useExtensionsManager.js` - توحيد مطابقة الإضافات بين نتائج OpenVSX والمثبت محلياً حتى تظهر حالة `Local` في القائمة والتفاصيل بنفس الوقت.
- [2026-05-26 21:30:00 +03] system:feature `server/services/packageService.js` و`client/src/components/EditorWorkspace.jsx` - استخراج manifest من ملفات VSIX وعرض capabilities مثل commands/themes/languages بعد التثبيت المحلي مع تجاهل مجلد `server/state/extensions`.
- [2026-05-26 21:18:00 +03] system:fix `server/services/packageService.js` و`server/routes/packages.js` و`client/src/hooks/useExtensionsManager.js` - تحويل تثبيت OpenVSX من success وهمي إلى تنزيل VSIX محلي محفوظ مع قائمة تثبيت persistent ورسالة UI صريحة أن التفعيل runtime لم يكتمل بعد.
