...
export function DiffApprovalModal({
  ...
  useEffect(() => {
    if (!currentDiffFile) return undefined;
  ...
  return (
    <div
      ...
      style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label={`Approve changes for ${getSavedFileName(currentDiffFile)}`}
        style={{ width: 'min(760px, 100%)', maxHeight: '80vh', background: t.sidebar, border: `0.5px solid ${t.border}`, borderRadius: 8, boxShadow: '0 18px 60px rgba(0,0,0,0.55)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
      >
        <div style={{ padding: '12px 14px', borderBottom: `0.5px solid ${t.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span dir="auto" style={bidiIsolateStyle({ fontSize: 13, color: t.text, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' })}>{getSavedFileName(currentDiffFile)}</span>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', background: t.bg, padding: 12, fontFamily: 'JetBrains Mono, Consolas, monospace' }}>
          {currentDiffLines.length === 0 ? (
            ...
            <div key={`${i}:${line}`} dir="ltr" style={codeTextStyle({ fontSize: 12, lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: line.startsWith('-') ? '#ff7b72' : line.startsWith('+') ? '#7ee787' : t.text })}>
              {line}
            </div>
          ))}
        </div>
